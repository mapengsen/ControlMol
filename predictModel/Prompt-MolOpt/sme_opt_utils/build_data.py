...
def atom_features (atom ,use_chirality =True ):
    ...
    [0 ,1 ,2 ,3 ,4 ,5 ,6 ])+[atom .GetFormalCharge (),atom .GetNumRadicalElectrons ()]+one_of_k_encoding_unk (atom .GetHybridization (),[
...
def substructure_features (m ,substructure ):
    ...
    # Chemical Keys Split Single Keys Double Three. Arrow Keys, count
BT_MAPPING_INT ={
...
def generate_substructure_features (mol ,substruct ):
    """ Generates a vector with mapping for a substructure: param substruct: The given substructure: param mol: RDKit molecule: param structure_type: The type of a structure (one of STRUCT_TO_NUM): return: An encoding vector """
    ...
    # implicit_substruct_valence CalculateTotalCount, Single Key As1, Double2, Three. 3, Arrow Keys1.5, Like: The benzene ring is9, Attention. , Calculate
    ...
    # substruct_valence Substructural Co-perpetrator, andOther
    substruct_valence =sum (atom .GetExplicitValence ()for atom in atoms )-2 *implicit_substruct_valence 

    # Substructure packs do not contain fragrance.
...
def construct_RGCN_mol_graph_from_smiles (smiles ,smask ,mol =None ):
    g =DGLGraph ()
    # Add nodes
    if mol is None :
...
def return_murcko_res_structure (smiles ):
    ...
    for sub_idx in sub_idx_list :
        ...
        # Delete it first. smiMedium*No. No.
...
# 39 function group config
    ...
    if for_mol ==True :
        ...
        # murcko graph
            ...
            # do not use the core and the whole mol, when mol opt
            m =Chem .MolFromSmiles (smiles )
            all_murcko_bond ,core =find_murcko_link_bond (m )
            res ,unmatched =rdRGD .RGroupDecompose ([core ],[m ],asSmiles =True )
...
# According toindexMerge, andAccording toRemainingindexMerge. WillA molecule. .
    emerge_sub_index_1 =[]
    emerge_sub_index_2 =[]
